...
def compute_classification_error(net, save_dir, dataset, layer, gt):
    ...
    accuracy_rgbd = 0
    cat_num = 11
    confusion_mat = np.zeros([cat_num, cat_num])
    ...
    for idx in dataset:
        ...
        rgb_prediction = (net.blobs['rgbd_'+layer].data).argmax()
	depth_prediction = (net.blobs['rgbd_'+layer].data).argmax()
	...
	if rgbd_prediction == label:
        ...
        loss_rgb += net.blobs['rgbd_loss'].data.flat[0]
	loss_depth += net.blobs['rgbd_loss'].data.flat[0]
	loss_rgbd += net.blobs['rgbd_loss'].data.flat[0]

        confusion_mat[label,rgbd_prediction] += 1


    return accuracy_rgb/len(dataset), accuracy_depth/len(dataset), accuracy_rgbd/len(dataset), loss_rgb/len(dataset), loss_depth/len(dataset), loss_rgbd/len(dataset), confusion_mat/(np.tile(confusion_mat.sum(1),(cat_num,1))).T
