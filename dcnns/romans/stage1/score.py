...
def compute_classification_error(net, save_dir, dataset, layer, gt):
	...
	loss_depth += net.blobs['depth_loss'].data.flat[0]
        ...
        confusion_mat[label,depth_prediction] += 1


    return accuracy_rgb/len(dataset), accuracy_depth/len(dataset), loss_rgb/len(dataset), loss_depth/len(dataset), confusion_mat/(np.tile(confusion_mat.sum(1),(cat_num,1))).T

def do_tests(net, iter, save_dir, test_indices, layer='fc8', gt='label'):

    accuracy_rgb, accuracy_depth, loss_rgb, loss_depth, confusion_mat = compute_classification_error(net, save_dir, test_indices, layer, gt)
