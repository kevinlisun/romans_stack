...
# @Last Modified time: 2017-02-23 17:46:57
...
class Detection_Recognition:
	...
	def __init__(self):
		...
		self.fid_sub = message_filters.Subscriber( FID_TOPIC, FrameID, queue_size=1 )
		ts = message_filters.ApproximateTimeSynchronizer([self.rgb_sub, self.depth_sub, self.fid_sub], 1, 1)
	...
	def doDectionAndRecognition( self, rgb, depth, fid ):
		...
		try:
			...
			#instance_map_msg = d_res.proposals.instance_map
			#bridge = CvBridge()
			#instance_map_cv = bridge.imgmsg_to_cv2( instance_map_msg, desired_encoding="passthrough" )
			#instance_map = np.asarray(instance_map_cv, dtype=np.int8)
			#print instance_map.shape 
			#bboxes_msg = d_res.proposals.bboxes
		...
		try:
			...
			if sys.argv[2] == 'svm':
				rospy.loginfo("extract feauture ...")
				extract_feauture = rospy.ServiceProxy( FEATURE_EXTRACTOR_SRV, FeatureExtractor )
				r_res = extract_feauture( rgb, d_res.masked_depth, d_res.proposals.bboxes, String('rgb_only') )

				feat = np.asarray(r_res.features.data, dtype=np.float32)
				feat = feat.reshape(r_res.features.height, r_res.features.width)

				start_time = time.time()

				svm = svm_load_model('/home/kevin/catkin_ws/src/romans_stack/odr/models/svm/svm_model.model')
				label = np.zeros(feat.shape[0])
				p_label, p_acc, p_val = svm_predict(label.tolist(), feat.tolist(), svm, '-b 0')

				print "SVM running time: " + str(time.time()-start_time)
				start_time = time.time()
			...
			# colored_semantic_map = colormap.getColoredSemanticmap(semantic_map)
			# colored_semantic_map_msg = bridge.cv2_to_imgmsg(colored_semantic_map, encoding="passthrough")
			# colored_semantic_map_msg.header = depth.header
			...
			result_folder = '/home/kevin/romans_bagfiles/evaluation2/' + sys.argv[3] + '/prediction'
			if os.path.isdir(result_folder) == False:
				os.mkdir(result_folder)
			
			io.savemat(os.path.join(result_folder, str(fid.data))+'.mat', {'semantic_map':np.array(semantic_map)})
			bridge = CvBridge()
			cv_img = bridge.imgmsg_to_cv2( v_res.colored_semantic_map, desired_encoding="passthrough" )
			np_img = np.asarray( cv_img )
			np_img = np_img[:,:,::-1] # if the image is bgr
			misc.imsave(os.path.join(result_folder, str(fid.data))+'.jpg', np_img)
