...
def main():
	...
	for foldi in folder:
		...
		for sp, idx in zip(sorted_probs,index):
			if sp > threshold:
				src = os.path.join(unlabelled_data_dir, files[idx] + '.png')
				dst = os.path.join(gp_labelled_data_dir, foldi, files[idx] + '.png')
				print src
				print dst
				shutil.copy(src, dst)

				src = '{}/{}.{}'.format(unlabelled_data_dir, files[idx] + '_depth', 'png')
				dst = '{}/{}/{}.{}'.format(gp_labelled_data_dir, foldi, files[idx] + '_depth', 'png')
