...
void filter_at_scale(float* depth_h,
	...
	int num_missing_pixels = get_missing_pixel_coords(height, width, mask_h, abs_inds_to_filter_h);
  	...
  	#pragma omp parallel for private(iter, threadID)
  	for (iter = 0; iter < num_missing_pixels; ++iter) {
...
void cbf::cbf(int height, int width, uint8_t* depth, uint8_t* intensity,
  ...
  int N = height * width;
	...
	for (int nn = 0; nn < N; ++nn) {
    	...
    	/*std::cout << "mask: " << mask_ms_h[nn] << " ";        
    	std::cout << "intensity: " << intensity_ms_h[nn] << " ";
        std::cout << "depth: " << depth_ms_h[nn] << " ";
	...
	for (int nn = 0; nn < N; ++nn) {
		if (mask_ms_h[nn]) {
		    result[nn] = static_cast<uint8_t>(result_ms_h[nn] * 255.0);
		} else {
			result[nn] = depth[nn];
		}
	}
